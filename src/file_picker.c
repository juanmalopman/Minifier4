...
static void internalOpenDialog(_In_ StateGUI* pStateGUI, _In_ bool isFile, _Out_writes_z_(MAX_PATH) PSTR pathToUpdate)
{
    ...
    if (FAILED(hr)) {  appLogError("ERROR: initializeLibraryCOM() failed."); return; }
    ...
    do
    {
        ...
        if (FAILED(hr)) { appLogError("ERROR: CoCreateInstance() failed."); break; }

        // Get current options.
        DWORD dwOptions;
        hr = IFileOpenDialog_GetOptions(pFileOpen, &dwOptions);
        
        if (FAILED(hr)) { appLogError("ERROR: IFileOpenDialog_GetOptions() failed."); break; }
        ...
        if (FAILED(hr)) { appLogError("ERROR: IFileOpenDialog_SetOptions() failed."); break; }
        ...
        if (FAILED(hr)) { appLogError("ERROR: IFileOpenDialog_Show() failed."); break; }

        // Get the result (Shell Item)
        hr = IFileOpenDialog_GetResult(pFileOpen, &pItem);

        if (FAILED(hr)) { appLogError("ERROR: IFileOpenDialog_GetResult() failed."); break; }

        // Get the file system path
        hr = IShellItem_GetDisplayName(pItem, SIGDN_FILESYSPATH, &pszFilePath);

        if (FAILED(hr)) { appLogError("ERROR: IShellItem_GetDisplayName() failed."); break; }

        // Success.
        size_t convertedChars = 0;
        wcstombs_s(&convertedChars, pathToUpdate, MAX_PATH, pszFilePath, _TRUNCATE);        
